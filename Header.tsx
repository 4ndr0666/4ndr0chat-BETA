...
interface HeaderProps {
    ...
    onOpenDeliveryStagerModal: () => void;
    onOpenTargetAnalysisModal: () => void;
...
export const Header: React.FC<HeaderProps> = ({ 
    ...
    onOpenDeliveryStagerModal,
    onOpenTargetAnalysisModal,
    isToolsEnabled,
    onEndChatSession
}) => {
                ...
                {isMenuOpen && (
                    <div className="absolute left-0 z-20 mt-2 w-64 origin-top-left border border-[var(--hud-color)] bg-black/80 backdrop-blur-md focus:outline-none">
                        <div className="p-1" role="menu" aria-orientation="vertical">
                            ...
                            <MenuItem onClick={() => handleMenuClick(onOpenTargetAnalysisModal)} disabled={anyLoading}>
                                <CrosshairsIcon className="w-4 h-4" /> Target Analysis
                            </MenuItem>
                            <MenuItem onClick={() => handleMenuClick(onOpenTelemetryModal)} disabled={anyLoading}>
                                <TargetIcon className="w-4 h-4" /> Live Telemetry
                            </MenuItem>
                             <MenuItem onClick={() => handleMenuClick(onOpenDeliveryStagerModal)} disabled={anyLoading}>
                                <SkullIcon className="w-4 h-4" /> Delivery Stager
                            </MenuItem>
                            <MenuItem onClick={() => handleMenuClick(onOpenReportGenerator)} disabled={anyLoading}>
                                <ReportIcon className="w-4 h-4" /> Engagement Report
                            </MenuItem>
