...
// FIX: Renamed props interface to match component.
interface LiveReconModalProps {
  isOpen: boolean;
  onClose: () => void;
}

// FIX: Renamed component to LiveReconModal to match usage in App.tsx.
export const LiveReconModal: React.FC<LiveReconModalProps> = ({ isOpen, onClose }) => {
  ...
  const [probeScript, setProbeScript] = useState('');
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(probeScript).then(() => {
      setIsCopied(true);
    ...
    >
      ...
      >
        ...
        <div className="flex justify-between items-center flex-shrink-0 relative">
            <h2 id="telemetry-modal-title" className="text-xl">Live Telemetry Capture</h2>
            <button
                onClick={onClose}
        ...
        <div className="flex-grow mt-4 flex flex-col min-h-0 space-y-4">
          <div>
              <label htmlFor="target-url-telemetry" className="block text-sm uppercase tracking-wider text-[var(--hud-color-darker)] mb-1">
                  Target URL
              </label>
              <input
                  id="target-url-telemetry"
                  type="url"
            ...
            <div className="flex-grow flex flex-col min-h-0 space-y-4 animate-fade-in">
                <div className="relative flex-grow min-h-0">
                    <textarea
                        ...
                        aria-label="Generated telemetry probe script"
                    />
                ...
                <div className="flex-shrink-0 border border-[var(--hud-color-darkest)] bg-black/50 p-3 font-mono text-xs text-[var(--hud-color-darker)] space-y-2">
                    ...
                    <p>1. Copy the telemetry probe script.</p>
                    <p>2. Execute it in the target's developer console.</p>
                    <p>3. Interact with the target page to generate network traffic.</p>
                    <p>4. Call <code className="text-[var(--hud-color)] bg-black px-1">getTelemetrySnapshot()</code> in the console.</p>
                    <p>5. Copy the logged JSON output.</p>
                    <p>6. Paste JSON into the Engagement Report Generator.</p>
                </div>
            </div>
          )}
        </div>
