...
// FIX: Renamed props interface for consistency.
...
// FIX: Renamed component to AdversarialReportGenerator to match usage in App.tsx.
export const AdversarialReportGenerator: React.FC<AdversarialReportGeneratorProps> = ({
  ...
  const { targetHostname: contextHostname } = useAppContext();
  ...
  useEffect(() => {
    if (isOpen) {
        setReconData(initialContext || null);
        setTargetHostname(contextHostname || '');
        if (initialContext) {
            setFileName('Context from Target Analysis');
        } else {
            setFileName(null);
        }
    }
  }, [isOpen, initialContext, contextHostname]);
  ...
  const handleGenerateClick = () => {
    if (reconData && targetHostname) {
      ...
      const errorMessages = [];
      if (!reconData) errorMessages.push("telemetry data");
  ...
  const handleDownload = () => {
      ...
      a.href = url;
      a.download = 'engagement_report.md';
    ...
    >
      ...
      >
        ...
        <div className="flex justify-between items-center flex-shrink-0 relative">
            <h2 id="report-generator-title" className="text-xl">Engagement Report Generator</h2>
            <button
                onClick={onClose}
            ...
            <div className="flex flex-col items-center justify-center h-full space-y-4 animate-fade-in">
              {initialContext ? (
                  <div className="w-full">
                      <label htmlFor="recon-context" className="block text-sm uppercase tracking-wider text-[var(--hud-color-darker)] mb-1">
                          Intelligence Context
                      </label>
                      <textarea
                          id="recon-context"
                          value={reconData || ''}
                          onChange={(e) => setReconData(e.target.value)}
                          className="w-full h-40 p-3 font-mono text-sm text-[var(--hud-color)] bg-black border border-[var(--hud-color-darkest)] focus:outline-none focus:ring-1 focus:ring-[var(--hud-color)]"
                      />
                  </div>
              ) : (
                <div
                ...
                >
                    ...
                    <label htmlFor="file-upload" className="cursor-pointer">
                        <p className="text-lg text-[var(--hud-color-darker)]">Drop Captured Telemetry (JSON)</p>
