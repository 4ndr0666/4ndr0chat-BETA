import { LanguageOption, SupportedLanguage, ProfileOption, ReviewProfile, FeatureDecisionRecord, FinalizationSummary, Part } from './types.ts';
...
export const THREAT_VECTOR_SYSTEM_INSTRUCTION = `You are a senior penetration tester. Your task is to generate a strategic threat analysis report. You will be given a target URL and optionally a collection of intelligence files (recon data, images, notes). Your analysis must be sharp, actionable, and prioritized.

**Analysis Scenarios:**
- **URL Only:** Infer the technology stack, identify potential threat vectors, and generate actionable reconnaissance steps.
- **URL + Data Files:** Synthesize all provided data (JSON scans, text notes, screenshots) to formulate prioritized attack paths. Go beyond simple enumeration and suggest specific exploit classes or techniques to try based on the evidence.

**Output Format:**
Your entire output must be in well-structured Markdown.
-   **Executive Summary:** A brief, high-level overview of the target's posture and the most critical findings.
-   **Inferred/Confirmed Technology Stack:** A summary of the technologies.
-   **Prioritized Attack Paths / Threat Vectors:** A breakdown of the most likely vulnerabilities.
-   **Recommended Next Actions:** A checklist of specific, actionable steps.
-   **Internal Tool Recommendations:** Based on your findings, recommend specific tools from the "4ndr0debuger" suite (Live Recon, Exploit Stager, Adversarial Report Generator).

**CRITICAL: Actionable Intelligence Block**
At the very end of your response, and only at the end, outside of the main markdown body, you MUST include a special JSON code block containing actionable targets identified in your analysis. This block is for programmatic use.
- The JSON object must have two keys: \`suggestedEndpointsForExploitation\` (an array of strings representing vulnerable paths, e.g., ["/api/v1/debug", "/admin/upload"]) and \`suggestedTargetsForLiveRecon\` (an array of strings representing hostnames or subdomains for further recon, e.g., ["sub.example.com"]).
- If no specific targets are found for a category, provide an empty array.

Example:
\`\`\`json
{
  "suggestedEndpointsForExploitation": ["/api/v1/user/details?id=1"],
  "suggestedTargetsForLiveRecon": []
}
\`\`\`
`;

export const generateThreatVectorPrompt = (options: { targetUrl: string; context?: string; parts?: Part[] }): { parts: Part[] } => {
    const { targetUrl, context, parts = [] } = options;
    
    let mainText = '';

    if (context) {
        mainText = `## Deeper Threat Vector Analysis Task\n\n**Target:** \`${targetUrl}\`\n\nBased on the following previous analysis, perform a deeper investigation into the most critical threat vectors mentioned. Expand on the recommended actions and suggest specific tools or commands where possible.\n\n**Previous Analysis Context:**\n---\n${context}\n---`;
    } else if (parts.length > 0) {
        mainText = `## Strategic Analysis Task from Provided Data\n\n**Target:** \`${targetUrl}\`\n\nAnalyze the following data files and images. Synthesize the findings into a strategic, prioritized plan for the next steps of a penetration test.`;
    } else {
        mainText = `## Threat Vector Analysis Task\n\n**Target URL:** \`${targetUrl}\`\n\nPlease provide an initial threat vector analysis for the target URL above.`;
    }
    
    const allParts: Part[] = [{ text: mainText }, ...parts];
...
export const SCOUT_SCRIPT_TEMPLATE = `// 4ndr0debug Live Recon Scout v2.2 (Stealth Update)
(() => {
    "use strict";
    ...
    const log = (msg) => console.log("%c[SCOUT v2.2]", "color: #ff003c; font-weight: bold;", msg);
    const error = (...args) => console.error("[SCOUT v2.2]", ...args);
    ...
    window.getTelemetrySnapshot = () => {
        log("Dumping captured session data.");
        ...
        console.log("copy(getTelemetrySnapshot.data)");
        window.getTelemetrySnapshot.data = dataToLog; // Attach data to function for easy access
        return "Data staged. See console for copy command.";
    };

    log("Scout injected. Network traffic is being monitored.");
    log("When ready, call 'getTelemetrySnapshot()' in the console to exfiltrate data.");
})();
`;
