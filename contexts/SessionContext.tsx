...
interface SessionContextType {
  ...
  handleThreatVectorAnalysis: (options: { targetUrl: string; context?: string; parts?: Part[]; }) => Promise<void>;
...
export const SessionProvider: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
    ...
    const handleThreatVectorAnalysis = useCallback(async (options: { targetUrl: string; context?: string; parts?: Part[]; }) => {
        if (!geminiService.isConfigured()) {
            addToast("API Key not configured.", "error");
            return;
        }
        ...
        const promptContents = generateThreatVectorPrompt(options);
