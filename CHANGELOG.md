...
### Added
- **Target Analysis Module**: A new "Target Analysis" tool, accessible from the command palette, for proactive reconnaissance against a target URL.
- **AI-Driven Analysis**: The module leverages the Gemini API to perform an intelligent analysis of a target's likely technology stack based on its URL and common web patterns.
- **Actionable Reporting**: Based on the inferred tech stack, the AI generates a structured Markdown report detailing potential vectors, common misconfigurations, and specific files or endpoints to investigate, providing a clear, actionable plan.

## [2.7.0] - 2024-08-27

### Added
- **Delivery Stager Input Validation**: The Delivery Stager now validates user inputs for Target Endpoint, LHOST, and LPORT, preventing common operator errors.
- **Expanded Payload Templates**: Added new templates to the Delivery Stager for greater flexibility.

### Changed
- **Enhanced Telemetry Probe**: The script now performs basic on-the-fly analysis of captured network traffic, heuristically identifying potential API endpoints and extracting interesting data fields.
- **Expanded Synthesis Heuristics**: The keyword matrix for the Dynamic Script Generation Engine has been expanded to better identify endpoints related to session management and file uploads.

## [2.6.0] - 2024-08-26

### Added
- **Delivery Stager Module**: A new tool accessible from the command palette designed to operationalize discovered server-side vulnerabilities.
- **Command Generation**: The Delivery Stager allows an operator to input vulnerability details and listener information, then generates a ready-to-use command to deliver a payload.
- **Payload Templates**: The stager includes pre-built templates for common payloads.

## [2.5.0] - 2024-08-25

### Changed
- **Upgraded Telemetry Probe**: The script generated by the Live Telemetry module has been overhauled for improved stealth and resilience.
- **Improved Data Exfiltration**: The exfiltration process for the probe has been made more reliable with clearer console instructions.

## [2.4.0] - 2024-08-24

### Added
- **Live Telemetry Module**: Implemented a new "Live Telemetry" tool, accessible from the command palette. This feature generates a disposable 'probe' script for active, in-browser data capture.
- **Probe Script Generation**: The new modal allows an operator to generate a lightweight, stealthy probe designed to be executed in a target's developer console to capture network traffic.
- **Manual Data Exfiltration**: The probe script exposes a `window.getTelemetrySnapshot()` function, which logs all captured intelligence to the console as a single JSON object.

## [2.3.0] - 2024-08-23

### Added
- **Parseltongue Polymorphic Engine**: Integrated the external `parseltongue.js` library to provide advanced, client-side script obfuscation.
- **Operator Obfuscation Control**: The Dynamic Script Generation Engine now features a "Polymorphic Obfuscation" toggle, giving the operator direct control over the script's stealth characteristics.

### Changed
- The script synthesis workflow now includes an optional final stage where the synthesized userscript is passed through the Parseltongue engine, randomizing its signature.

## [2.2.0] - 2024-08-22

### Added
- **Dynamic Script Generation Engine**: Replaced the legacy synthesizer. This new engine implements advanced heuristic analysis.
- **Advanced Heuristic Analysis**: The engine now uses a weighted keyword matrix and advanced tokenization to analyze target codebases, dramatically increasing the accuracy of identifying high-value targets.
- **Enhanced Operator Feedback Loop**: The engine's status log provides a detailed, real-time briefing on its findings, including confidence scores.

### Changed
- **UI/UX Overhaul**: The synthesizer interface has been updated with clearer labeling and a command-centric action button `[ Analyze & Generate Script ]`.

## [2.1.0] - 2024-08-21

### Added
- **Persistent Mode Selector**: A new segmented control UI has been implemented directly below the header for faster switching between Debug, Single Review, Compare, and Audit modes.

### Changed
- **Streamlined Command Palette**: The mode-switching options have been removed from the command palette menu, elevating mode selection to a primary UI element.
- **Enhanced Visual Feedback**: The active mode in the new segmented control features a subtle pulse animation.
...
### Fixed
- **System Cohesion**: Performed a full-system audit to validate that all features introduced in the v2.0.0 roadmap are fully implemented, wired correctly into the command palette, and are production-ready. All major workflows, including Debug, Compare & Revise, Audit, and Script Synthesis, are confirmed to be stable and complete.
...
### Fixed
- **Codebase Cohesion**: Consolidated the script synthesizer component by implementing the `PayloadSynthesizer` and updating all references, resolving an inconsistency between the codebase and the changelog.
...
- **Fileless Script Delivery**: The `EngineSynthesizer` now features a "Stage for Delivery" option. This creates a temporary, in-memory blob URL for the synthesized script and provides a single-line `import(...)` loader command, enabling a stealthy, fileless delivery mechanism.
